...
import { FileBarChart, Download, Receipt, IndianRupee, BookOpen, ShoppingCart, BarChart2, FileText } from "lucide-react";
...
export default function Reports() {
  ...
  useEffect(() => { api.get("/reports/daily", { params: { day } }).then((r) => setDaily(r.data)).catch(() => setDaily(null)); }, [day]);
  useEffect(() => { api.get("/reports/monthly", { params: { month } }).then((r) => setMonthly(r.data)).catch(() => setMonthly(null)); }, [month]);
  useEffect(() => { api.get("/shops").then((r) => setShops(r.data)).catch(() => setShops([])); }, []);
  useEffect(() => {
    if (stShop) api.get(`/statement/${stShop}`, { params: { start: stStart, end: stEnd } }).then((r) => setStatement(r.data)).catch(() => setStatement(null));
  ...
  return (
    <div>
      ...
      {/* Constant 4 Summary Cards Across All Tabs */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Stat 
          icon={ShoppingCart} 
          label="Purchase Invoice" 
          value={inr(tab === "daily" ? (daily?.purchase_invoice_total ?? 0) : tab === "monthly" ? (monthly?.purchase_invoice_total ?? 0) : (statement?.total_purchase ?? 0))} 
          accent="bg-red-500/15 text-red-300" 
        />
        <Stat 
          icon={FileText} 
          label="Sale Invoice" 
          value={inr(tab === "daily" ? (daily?.sale_invoice_total ?? 0) : tab === "monthly" ? (monthly?.sale_invoice_total ?? 0) : (statement?.total_sale ?? 0))} 
          accent="bg-blue-500/15 text-blue-300" 
        />
        <Stat 
          icon={Receipt} 
          label="Sale GST Summary" 
          value={inr(tab === "daily" ? (daily?.gst_collected_sales ?? 0) : tab === "monthly" ? (monthly?.gst_collected_sales ?? 0) : (statement?.gst_collected_sales ?? 0))} 
          accent="bg-emerald-500/15 text-emerald-300" 
        />
        <Stat 
          icon={BarChart2} 
          label="Purchase GST Summary" 
          value={inr(tab === "daily" ? (daily?.gst_collected_purchase ?? 0) : tab === "monthly" ? (monthly?.gst_collected_purchase ?? 0) : (statement?.gst_collected_purchase ?? 0))} 
      ...
      {tab === "daily" && daily && (
        <div data-testid="daily-report">
          <div className="flex items-center gap-3 mb-4">
            ...
            <button data-testid="export-daily-button" onClick={() => exportToCsv(`daily-${day}.csv`, [{ ...daily }], [{ label: "Day", accessor: "day" }, { label: "Purchase Invoice", accessor: "purchase_invoice_total" }, { label: "Sale Invoice", accessor: "sale_invoice_total" }, { label: "Sale GST", accessor: "gst_collected_sales" }, { label: "Purchase GST", accessor: "gst_collected_purchase" }])} className="ml-auto flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm hover:bg-white/10 transition"><Download className="h-4 w-4" /> Export</button>
          </div>
          ...
          <Card className="overflow-hidden">
            ...
            <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-white/10 text-left text-xs uppercase tracking-wider text-slate-500 font-mono"><th className="p-4">Invoice</th><th className="p-4">Type</th><th className="p-4">Shop</th><th className="p-4 text-right">Total</th></tr></thead>
              <tbody className="divide-y divide-white/5">
                {(daily?.invoices || []).map((i) => <tr key={i.id} className="hover:bg-white/5"><td className="p-4 font-mono text-cyan-300">{i.invoice_no}</td><td className="p-4"><span className={`px-2 py-0.5 rounded text-xs font-semibold ${i.invoice_type === 'PURCHASE' ? 'bg-red-500/20 text-red-300' : 'bg-blue-500/20 text-blue-300'}`}>{i.invoice_type || 'SALES'}</span></td><td className="p-4">{i.shop_no} · {i.shop_name}</td><td className="p-4 text-right font-mono">{inr(i.grand_total)}</td></tr>)}
                {(!daily?.invoices || daily.invoices.length === 0) && <tr><td colSpan={4} className="p-6 text-center text-slate-500">No invoices this day.</td></tr>}
              </tbody></table></div>
          </Card>
        </div>
      )}
        ...
        <div data-testid="monthly-report" id="monthly-report-sheet">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <button data-testid="export-monthly-button" onClick={() => exportToCsv(`monthly-${month}.csv`, monthly?.daily || [], [{ label: "Date", accessor: "date" }, { label: "Sale", accessor: "sale" }, { label: "Purchase", accessor: "purchase" }])} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm hover:bg-white/10 transition"><Download className="h-4 w-4" /> CSV</button>
            </div>
          </div>
            ...
            <div style={{ width: "100%", height: 300 }} data-testid="monthly-chart">
              <ResponsiveContainer>
                <BarChart data={monthly?.daily || []} margin={{ left: -10, right: 10 }}>
                  ...
                  <Bar dataKey="sale" fill="#06b6d4" radius={[6, 6, 0, 0]} name="Sale (₹)" />
                  <Bar dataKey="purchase" fill="#ef4444" radius={[6, 6, 0, 0]} name="Purchase (₹)" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </Card>
            ...
            <Card className="p-0 overflow-hidden">
              <div id="statement-sheet" className="bg-white text-slate-900 p-8">
                ...
                <table className="w-full text-sm border border-slate-300" data-testid="statement-table">
                  ...
                  <tbody>
                    {/* <tr><td className="p-2 border border-slate-300 text-slate-500" colSpan={4}>Opening Balance (as of {fmtDate(statement?.start)})</td><td className="p-2 border border-slate-300 text-right font-mono">{inr(statement?.opening_balance ?? 0)}</td></tr> */}
                    {(statement?.rows || []).map((r, i) => (
                      <tr key={i}>
                        ...
                        <td className="p-2 border border-slate-300 text-right font-mono">{Number(r.balance || 0).toFixed(2)}</td>
                      </tr>
                    ))}
                    {(!statement?.rows || statement.rows.length === 0) && <tr><td className="p-3 border border-slate-300 text-center text-slate-500" colSpan={5}>No transactions in this period.</td></tr>}
                    <tr className="font-semibold bg-slate-50">
                      <td className="p-2 border border-slate-300 text-right" colSpan={2}>Totals</td>
                      <td className="p-2 border border-slate-300 text-right font-mono">{(statement?.total_debit ?? 0).toFixed(2)}</td>
                      <td className="p-2 border border-slate-300 text-right font-mono">{(statement?.total_credit ?? 0).toFixed(2)}</td>
                      <td className="p-2 border border-slate-300 text-right font-mono" data-testid="statement-closing">{(statement?.closing_balance ?? 0).toFixed(2)}</td>
                    </tr>
                  </tbody>
                </table>
                <p className="text-right text-sm font-bold mt-3">Closing Balance Due: ₹ {(statement?.closing_balance ?? 0).toFixed(2)}</p>
